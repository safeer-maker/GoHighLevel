...
| **Quiet hours respected** | Yes — all marketing SMS/email respect 8 AM – 9 PM contact-local |
...
| 1e | Send SMS | Template `02 — Day 0 — Trial Welcome SMS` |
...
### Action 3 — Day 1 SMS (Attendance-Branching)

| Property | Value |
|---|---|
| **Action type** | If / Else |
| **Condition** | `last_visit_date` >= `lead_captured_at` AND `total_visits_lifetime` >= 1 |
| **YES branch** | Send SMS `02 — Day 1 — Class Confirmation` |
| **NO branch** | Send SMS `02 — Day 1 — Booking Check` |
...
### Action 7 — Day 4 Email + Conditional Nudge SMS
...
| 7c-NO | Send SMS | `02 — Day 4 — Class Nudge` |
...
### Action 10 — Wait Until Day 6, 2 PM → Personal SMS
...
| 10b-NO | Send SMS | `02 — Day 6 — Personal from Morgan` |

**Why 2 PM specifically:** Afternoon is the highest-reply window for personal-tone SMS. Morning gets buried, evening reads as desperate.
...
| 12c-NO | Send SMS | `02 — Day 7 — Last Call SMS` |
...
| 14c-6 | (Optional) Send SMS | `02 — Day 8 — Soft Goodbye` |
...
```mermaid
graph TD
    T[Tag Added: trial-claimed] --> A1[Day 0: Setup + Welcome Email + SMS]
    A1 --> A2[Wait until Day 1, 10 AM]
    A2 --> A3{Attended yet?}
    A3 -->|Yes| A3y[SMS: Class Confirmation]
    A3 -->|No| A3n[SMS: Booking Check]
...
| Contact has `do-not-sms` | All SMS actions skip silently. Email actions still fire. |
| Contact has `do-not-email` | All Email actions skip silently. SMS actions still fire. |
...
| Contact never opens an email or SMS | Workflow runs full sequence. Likely outcome: `trial-expired` (silent expiration). |
