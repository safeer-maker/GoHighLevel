...
## Step 3 — Build Email Templates (15 min)

Navigate to **Marketing > Templates > Email > + New Template**.

| Template Name | Send Day | Used in workflow step |
|---|---|---|
| `04 — Day 1 — Personal Welcome Email` | Day 1 | Step 4.4 |
...
(No Day 0 Email — they just got the conversion-confirmation Email from #02. Don't double-send within the hour.)

### 4.3 Action: Wait until Day 1, 10 AM contact-local

- **Wait Until:** `membership_start_date + 1 day` at 10:00 AM contact-local

### 4.4 Action: Day 1 Personal Email

- **Send Email:** `04 — Day 1 — Personal Welcome Email` (from Morgan, personal-tone)
...
- **If/Else:** `last_visit_date` >= `membership_start_date` (have they visited since signup?)
  ...
  - **No:** Send Email `04 — Day 3 — No Visit Yet (gentle)`. Add tag `onboarding-needs-nudge`.
...
- **If/Else:** `membership_tier` =
  ...
  - **Premium / VIP:** Send Email: *"Hey {{first_name}} — your membership includes PT sessions. Want me to set up your first one with {{contact.assigned_trainer}}? Just reply with a day/time."*
...
- **If/Else:** `visits_last_30_days` (since membership_start_date) — how many?
  - **3+ visits → Healthy path:**
    ...
    - Send Email `04 — Day 14 — Milestone Celebrate`
...
- **If/Else:** still zero visits AND `risk-critical`?
  - **Yes:** Send personal Email from Morgan: *"Hey {{first_name}}, Morgan here. Wanted to check in — you joined 17 days ago and haven't been able to come in yet. Totally happens. Anything I can do? Even a 10-min phone call works."*
  - **No:** Skip.
...
- **Wait 6 hours**
- **Send Email:** `04 — Day 21 — Goal Review Reminder`
...
- **If/Else:** Has the member booked the goal review? (Check: any appointment on `Goal Review Calendar` between days 21-30 for this contact)
  ...
  - **No:** Send Email: *"{{first_name}}, your goal review with {{contact.assigned_trainer}} is still open — grab 30 min: {{custom_values.business.booking_url}}"*
...
When a member hits `risk-critical` during onboarding, Morgan needs a one-button way to fire the save Email without manually typing.

Navigate to **Automation > Workflows > + Create Workflow > Start from Scratch**.

- **Workflow Name:** `04 — Onboarding Save Email (Owner-Triggered)`
...
1. Send Email from Morgan: *"Hey {{first_name}}, Morgan here personally — I noticed you joined 2 weeks ago and haven't been able to make it in. Totally happens; life is a thing. If schedule is the problem, just email me back with what time of day works for you and I'll find you a class. If something else is in the way, I'd love to know — I read every reply. — Morgan"*
...
2. **Expected:** Day 1 personal welcome Email arrives.
...
3. **Expected:** No Day 3 Email fires (they've attended). Tag `onboarding-attended-week1`.
4. Test contact B: no visits.
5. Fast-forward to Day 3.
6. **Expected:** Day 3 "no visit yet" Email fires. Tag `onboarding-needs-nudge` applied.

### Test 4 — Day 14 three-way branch

1. Test contact A: simulate 3 visits since signup.
2. Fast-forward to Day 14.
3. **Expected:** Healthy path — Day 14 milestone emails, tag `risk-healthy`.
...
3. **Expected:** Personal Email from Morgan fires.

### Test 6 — Day 21 goal review

1. Fast-forward any test contact to Day 21.
2. **Expected:** Goal review invite emails arrive. Opportunity stage → "Goal Review (Day 21)".

### Test 7 — Day 30 graduation + handoff

1. Test contact: 3+ visits, fast-forward to Day 30.
2. **Expected:**
   - Day 30 graduation emails arrive.
...
### Test 9 — Owner save Email

1. On a `risk-critical` test contact, manually add tag `owner-save-sms`.
2. **Expected:** Save Email from Morgan fires immediately. Tag `onboarding-save-attempted` applied.
...
3. **Quiet hours violated on Day 1 Email.** A member who converts at 9 PM will get a Day 1 Email the next morning at 10 AM — good. But if the workflow fires the Day 1 Email *immediately* on the day-after wait, check that 10 AM contact-local is honored, not server time.
